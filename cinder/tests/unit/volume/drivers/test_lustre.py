# Copyright (c) 2016 DataDirect Networks Storage, Inc.
# All Rights Reserved.
#
...
#    under the License.
"""Unit tests for the Lustre driver module."""
...
class LustreDriverTestCase(test.TestCase):
    """Test case for Lustre driver."""

    TEST_LUSTRE_HOST = 'lustre-host1@tcp'
    TEST_LUSTRE_SHARE_PATH = '/lustre'
    TEST_EXPORT1 = 'lustre-host1@tcp:/lustre'
    TEST_EXPORT2 = 'lustre-host2@tcp:/lustre'
    TEST_EXPORT2_OPTIONS = '-o backupvolfile-server=lustre-backup1'
    TEST_SIZE_IN_GB = 1
    TEST_MNT_POINT = '/mnt/lustre'
    ...
    def setUp(self):
        super(LustreDriverTestCase, self).setUp()
        self._configuration = mock.MagicMock()
        self._configuration.lustre_mount_point_base = (
            self.TEST_MNT_POINT_BASE)
        self._configuration.nas_secure_file_permissions = 'false'
        self._configuration.nas_secure_file_operations = 'false'
        self._configuration.nas_host = 'lustre-host1@tcp'
        self._configuration.nas_share_path = '/lustre'
        self._configuration.nas_mount_options = None
        self._driver =\
            lustre.LustreDriver(configuration=self._configuration,
                                db=FakeDb())
        self._driver.shares = {}
    ...
    def test_local_path(self):
        ...
        self.override_config("lustre_mount_point_base",
                             self.TEST_MNT_POINT_BASE)
        drv = self._driver
        ...
        self.assertEqual(
            '/mnt/test/3acc2372f72d0d318f1de8bd3d4f3196/%s' % volume.name,
            drv.local_path(volume))

    def test_mount_lustre(self):
        """_mount_lustre common case usage."""
        drv = self._driver

        with mock.patch.object(os_brick.remotefs.remotefs.RemoteFsClient,
                               'mount') as mock_mount:
            drv._mount_lustre(self.TEST_EXPORT1)

            mock_mount.assert_called_once_with(self.TEST_EXPORT1, [])

    def test_mount_lustre_should_reraise_exception_on_failure(self):
        """_mount_lustre should reraise exception if mount fails."""
        drv = self._driver

        with mock.patch.object(os_brick.remotefs.remotefs.RemoteFsClient,
                               'mount') as mock_mount:

            mock_mount.side_effect = exception.LustreException()

            self.assertRaises(exception.LustreException,
                              drv._mount_lustre, self.TEST_EXPORT1)
    ...
    def test_get_hash_str(self):
        ...
        self.assertEqual('3acc2372f72d0d318f1de8bd3d4f3196',
    ...
    def test_get_available_capacity_with_df(self):
        ...
        df_data = 'lustre-host@tcp:/lustre %d 996864 %d 41%% /mnt' % \
    ...
    def test_get_provisioned_capacity(self):
        ...
        drv.shares = {'127.7.7.7:/lustre1': None}
        with mock.patch.object(drv, '_get_mount_point_for_share') as \
                mock_get_mount_point_for_share,\
                mock.patch.object(drv, '_execute') as mock_execute:
            mock_get_mount_point_for_share.\
    ...
    def test_load_shares_config_nas_opts(self):
        drv = self._driver
        drv.configuration.nas_host = self.TEST_LUSTRE_HOST
        drv.configuration.nas_share_path = self.TEST_LUSTRE_SHARE_PATH
    ...
    def test_ensure_share_mounted(self):
        ...
        with mock.patch.object(utils, 'get_file_mode') as \
                ...
                mock_get_mount_point_for_share,\
                mock.patch.object(drv, '_mount_lustre') as \
                mock_mount_lustre:
            ...
            self.assertTrue(mock_mount_lustre.called)
    ...
    def test_setup_should_throw_exception_if_client_is_not_installed(
        ...
        with mock.patch.object(drv, '_execute') as mock_execute:
            ...
            self.assertRaisesAndMessageMatches(exception.LustreException,
                                               'mount.lustre is not '
                                               'installed',
                                               drv.do_setup,
                                               mock.MagicMock())

    def _fake_load_shares_config(self):
        self._driver.shares = {'127.7.7.7:/lustre1': None}
    ...
    def test_setup_set_share_permissions(self, mock_exists, mock_getegid):
        ...
        with mock.patch.object(drv, '_execute') as mock_execute,\
            ...
            expected = [
                mock.call('mount.lustre', check_exit_code=False),
                mock.call('umount',
                          '/mnt/test/ee8ede17be0c591453c3b46168d73c47',
                          run_as_root=True),
                mock.call('chgrp', 888,
                          '/mnt/test/ee8ede17be0c591453c3b46168d73c47',
                          run_as_root=True),
            ...
            mock_mount.assert_called_once_with('127.7.7.7:/lustre1', [])
    ...
    def test_find_share_should_throw_error_if_there_is_no_mounted_shares(self):
        ...
        self.assertRaises(exception.LustreNoSharesMounted,
                          drv._find_share,
    ...
    def test_find_share_should_throw_error_if_there_is_no_enough_place(self):
        ...
        with mock.patch.object(drv, '_get_available_capacity') as \
            ...
            self.assertRaises(exception.LustreNoSuitableShareFound,
                              drv._find_share,
                              self.TEST_SIZE_IN_GB)
    ...
    def test_refresh_mounts_with_excp(self):
        with mock.patch.object(self._driver, '_unmount_shares') as \
                ...
                mock.patch.object(self._driver, '_ensure_shares_mounted') as \
                mock_ensure_shares_mounted,\
                mock.patch.object(lustre, 'LOG') as mock_logger:
    ...
    def test_unmount_shares_with_excp(self):
        ...
        with mock.patch.object(self._driver, '_load_shares_config') as \
                ...
                mock_do_umount,\
                mock.patch.object(lustre, 'LOG') as \
                mock_logger:
    ...
    def test_initialize_connection(self):
        ...
        with mock.patch.object(drv, 'get_active_image_from_info') as \
            ...
            self.assertEqual('lustre', conn_info['driver_volume_type'])
